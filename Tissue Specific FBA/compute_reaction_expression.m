...
for i=1:size(genes)
    lung(i)=length(genes{i});
end
[lung_sorted, ixs_genes_sorted_by_length] = sort(lung,'descend');

reaction_expression_aux = reaction_expression;

for i = 1:numel(ixs_genes_sorted_by_length)
    j = ixs_genes_sorted_by_length(i);
    matches = strfind(reaction_expression_aux,genes{j});     %this and the following instruction find the locations of the gene 'bXXXX' in the array reaction_expression
    pos_genes_in_react_expr{j} = find(~cellfun('isempty', matches));
    reaction_expression_aux(pos_genes_in_react_expr{j}) = strrep(reaction_expression_aux(pos_genes_in_react_expr{j}),genes{j},'');   %replaced with empty char so it's not found again later (this avoid for instance looking for 'HGNC:111' and replacing also partially another gene named for instance 'HGNC:1113'
end

%% everything follows is useless because it's all in fbamodel.genes
% we also prepare the array of genes
%genes = gpr2genes(fbamodel.grRules);
%genes = unique(genes); %removes duplicates, we need unique array of genes, without duplicates
